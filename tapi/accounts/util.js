...
const filterAccounts = (accounts, query, archived = false) =>
  accounts.accountDetails.filter((account) =>
    Object.entries(query).reduce(
      (pass, [k, v]) => pass && account[k] === v && (parseInt(account.archive_status, 10) === 1) === archived,
      true,
    ),
  );
