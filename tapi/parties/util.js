...
const hasRequiredPartyFields = (user) => requiredFields.reduce((acc, column) => acc && user[column] != null, true);
...
const userToParty = (user) =>
  Object.fromEntries(
    Object.entries({
      partyId: user.partyId,
      domicile: user.usa_citizenship_status ? citizenshipStatusToDomicile(user.usa_citizenship_status) : null,
      ...
      avgAnnIncome: user.current_income && user.prior_income ? (user.current_income + user.prior_income) / 200 : null,
      currentHouseholdIncome: user.household_income ? user.household_income / 100 : null,
      avgHouseholdIncome:
        user.household_current_income && user.household_prior_income
          ? (user.household_current_income + user.household_prior_income) / 200
          : null,
      householdNetworth: user.household_net_worth ? user.household_net_worth / 100 : null,
    }).filter(([, v]) => v != null),
  );

const filterParties = (parties, query, archived = false) =>
